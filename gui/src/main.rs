...
use chess_engine::{
    ...
    Board, Color, GameState, Move, Position,
};
...
struct PieceSprite;
struct Game {
    game: Arc<Mutex<chess_engine::Game>>,
...
fn display_promotion_screen(game: Res<Game>) -> ShouldRun {
...
fn should_init_host(mut game: ResMut<Game>) -> ShouldRun {
    if game.should_init && game.game_type == GameType::Host {
        game.should_init = false;
        ShouldRun::Yes
    } else {
        ShouldRun::No
    }
...
fn should_init_client(mut game: ResMut<Game>) -> ShouldRun {
    if game.should_init && game.game_type == GameType::Client{
        game.should_init = false;
        ShouldRun::Yes
    } else {
        ShouldRun::No
...
fn main() {
    ...
    App::build()
        ...
        //.add_startup_stage("server_setup", SystemStage::single(server_system.system()))
        .add_system_set(
            SystemSet::new()
        ...
        .add_system_set(
            SystemSet::new()
                .with_run_criteria(should_init_host.system())    
                .with_system(server_system.system())
        )
        .add_system_set(
            SystemSet::new()
                .with_run_criteria(should_init_client.system()) 
                .with_system(client_system.system())
        )
        .run();
...
fn server_system(mut server: ResMut<Server>, mut game: ResMut<Game>) {
    println!("Server system running");
    let olle = game.game.clone();
    let r = server.run(&olle);
    println!("Server Threads initialized, Result: {:?}", r);
}
...
fn handle_mouse_click_for_promotion(
    ...
    if mouse_input.just_pressed(MouseButton::Left) {
        ...
        } else if cursor_cordinate.x < window.width() / 2.
            && cursor_cordinate.y > window.height() / 2.
        {
...
fn exchange_promotion_piece(mut game: ResMut<Game>, kind: Kind) {
    ...
    let piece = Piece {
        color: get_other_color(game.game.lock().unwrap().clone().board().next_to_move()),
        kind,
    };
    game.game
        .lock()
        .unwrap()
        .unsafe_set_piece(pos.unwrap(), piece);
...
fn dummy_system(mut commands: Commands, game: Res<Game>, materials: Res<Materials>) {
    promotion_sprite_spawn(
        &mut commands,
        match_piece_to_sprite(
            Queen,
            get_other_color(game.game.lock().unwrap().board().next_to_move()),
            &materials,
        ),
        -150.,
        150.,
...
fn setup_system(
    ...
    let mut game = Arc::new(Mutex::new(chess_engine::Game::new(
    ...
    println!("Host or Client?");
    let mut game_type: GameType = GameType::Host;
...
fn pieces_spawn(
    ...
    let g = game.game.lock().unwrap();
    let tiles = g.board().tiles(); // TODO: REFACTOR
...
fn handle_mouse_clicks_system(
    ...
    if mouse_input.just_pressed(MouseButton::Left) {
        handle_hightlights(
            ...
            &mut materials,
            &mut commands,
            &mut highlighted_moves,
            &mut highlighted_piece,
            window,
        );
...
fn handle_moves(
    ...
    let b = game.game.lock().unwrap().board().clone();
    ...
    for dest_pos in moves {
        if cursor_pos == dest_pos {
            ...
            match game.game.lock().unwrap().make_move(m, || Queen) {
            ...
            let destination_piece = game.game.lock().unwrap().board()[dest_pos].unwrap();
            ...
            print!("{}", game.game.lock().unwrap().board().to_string());
        }
    }
}
...
fn handle_hightlights(
    ...
    let piece = match game.game.lock().unwrap().board()[pos] {
        Some(p) => p,
        None => return,
    };

    let game_type_color = match game.game_type {
        GameType::Client => Color::Black,
        GameType::Host => Color::White,
        GameType::Offline=> piece.color,
        _ => panic!("unknown game type"),
    };

    if piece.color == game.game.lock().unwrap().board().next_to_move() && piece.color == game_type_color {
        game.highlighted_piece = Some(PieceHighlight { piece, pos });
        ...
        // Highlight Moves
        let g = game.game.lock().unwrap();
        let moves = piece.moves(g.board(), pos);
        for dest_pos in moves {
...
fn highlight_piece_spawn_system(
    ...
    commands
        ...
        .insert(PieceHighlight { piece, pos });
}
